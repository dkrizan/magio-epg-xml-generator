...
import os
import json
from .programme import Programme, Channel
...
class IPTVClient(object):
    ...
    def epg(self, channels: List[Channel], from_date, days: int = 7) -> Dict[str, List[Programme]]:
        raise NotImplementedError("Should have implemented this")
...
class MagioQuality:
    ...
    @staticmethod
    def get(index):
        return {0: MagioQuality.low, 1: MagioQuality.medium, 2: MagioQuality.high, 3: MagioQuality.extra}.get(index,
                                                                                                              MagioQuality.high)
...
class MagioGo(IPTVClient):
    ...
    def __init__(self, user_name, password, storage_dir=".", quality=MagioQuality.extra):
        self._user_name = user_name
        self._password = password
        self._quality = quality
        self._device = 'Web Browser'
        self._data = MagioGoSessionData()
        super().__init__(storage_dir, 'temp/%s.session' % self._user_name)
    ...
    def epg(self, channels, from_date: datetime, days: int = 7) -> Dict[str, List[Programme]]:
            ...
            while fetch_more:
                ...
                for i in resp['items']:
                    for p in i['programs']:
                        ...
                        programme.is_replyable = (programme.start_time > (now - datetime.timedelta(days=7))) and (
    ...
    @staticmethod
    def export_epg(file_name: str, channels: List[Channel], epg: Dict[str, List[Programme]]):
